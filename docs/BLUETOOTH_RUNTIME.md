...
Управление system service выполняется через `sudo -n /usr/bin/systemctl` и требует
ограниченного sudoers-правила для пользователя приложения. Установите и
проверьте его скриптами `scripts/install-system-permissions.sh` и
`scripts/check-system-setup.sh`; подробности находятся в
`docs/BLUEALSA_APLAY_AUTHORIZATION.md`. При отказе в доступе ошибка
