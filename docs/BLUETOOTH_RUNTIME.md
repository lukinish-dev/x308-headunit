...
Управление system service требует разрешения systemd/polkit для пользователя,
под которым запускается приложение. В коде нет скрытого `sudo`; отказ в доступе
возвращается как ошибка, а active source не меняется. Политика polkit в этом
этапе не создавалась.
