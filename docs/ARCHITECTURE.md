...
`SystemStatus` пока не реализован. Целевая роль — read-only агрегация лёгких
данных от Configuration, `IMediaPlayer`, `IBluetoothManager`, `SourceManager` и
малых системных probes.
...
читает Linux-источники напрямую.
...
Текущий ProcessRunner использует `poll`, а не фоновые потоки. Временный pairing
agent принадлежит `BluetoothCtlManager`, имеет ограниченный срок жизни и
...
Ниже перечислены факты текущего кода. Они сознательно **не исправлялись** в
рамках документационной задачи.
...
9. **`SystemStatus` и команда `status` не реализованы.** Команда упомянута в
   help-тексте, но отдельного read-only application service и модели отчёта
   пока нет.

10. **Переключение источника не имеет rollback.** `SourceManager` обновляет
...
src/
  ...
  cli/                        CLI parser and interactive presentation
